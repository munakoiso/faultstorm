...
# the daemon reads process name patterns from it (one per line), finds matching
# PIDs, picks a random one, and enters a SIGSTOP/SIGCONT loop with random
# delays (0.1–3 seconds). When the flag is removed (heal), the loop exits and
# the daemon goes back to polling.
#
# Flag file format (one process name pattern per line):
#   postgres
#   pgconsul
#
...
# Generate a random float between 0.1 and 3.0 seconds.
# Uses $RANDOM (0-32767) to approximate.
random_delay() {
    local min_ms=100   # 0.1s
    local max_ms=3000  # 3.0s
    local range=$((max_ms - min_ms))
    local delay_ms=$(( (RANDOM % range) + min_ms ))
...
freeze_loop() {
    # Read patterns from the flag file
    local patterns=()
    while IFS= read -r line; do
    ...
    if [ ${#patterns[@]} -eq 0 ]; then
        log "WARNING: flag file is empty, no patterns to match"
        return
    fi
