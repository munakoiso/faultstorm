...
def add_drop_rules_by_src(node: str, chain_name: str,
...
def create_chain_for_direction(node: str, chain_name: str,
                               direction: str) -> None:
    ...
    try:
        ...
        logger.warning("create_chain_for_direction node=%s chain=%s dir=%s "
                       "error: %s", node, chain_name, direction, e)


def add_drop_rules_by_dest(node: str, chain_name: str,
    ...
    for ip in dest_ips:
        try:
            ...
            logger.warning("add_drop_by_dest node=%s chain=%s ip=%s error: %s",
                           node, chain_name, ip, e)


def remove_chain_for_direction(node: str, chain_name: str,
                               direction: str) -> None:
    ...
    try:
        ...
        logger.warning("remove_chain_for_direction node=%s chain=%s dir=%s "
                       "error: %s", node, chain_name, direction, e)


def heal_directional_partition(partition_id: int, node: str,
    ...
    logger.info("healed directional partition id=%d node=%s directions=%s",
                partition_id, node, directions)
